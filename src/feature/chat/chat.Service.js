...
const { CHAT } = require("../../utils/responseMessage");
const ObjectId = mongoose.Types.ObjectId;
const filename = "chat.Service.js";
module.exports = {
  ...
  updateRoom: async (requestObj) => {
    ...
    try {
      ...
      const data = await ChatModel.findByIdAndUpdate(_id, rest, {
        new: true,
      }).lean();
      if (!data) {
        result.error = CHAT.CHAT_NOT_FOUND;
        logger.error(CHAT.CHAT_NOT_FOUND + filename);
        return result;
      } else {
        result.data = data;
        return result;
  ...
  deleteRoom: async (id) => {
    ...
    try {
      ...
      if (!data) {
        logger.error(CHAT.SOME_ERROR_OCCURRED + filename);
        result.error = CHAT.SOME_ERROR_OCCURRED;
        return result;
      } else {
        result = [];
        return result;
  ...
  /**
   * @description : this function is used to remove the members frm is other array list
   * @param {*} messageObj
   */
