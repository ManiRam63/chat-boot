...
module.exports = {
    ...
    try {
      ...
      if (result?.error || result?.message) {
        ...
        if (result.message) {
          ...
          message = ROOM.SOME_ERROR_OCCURRED;
        }
        ...
        return successResponse(
          res,
          ROOM.ROOM_CREATED_SUCCESSFULLY,
          200,
          result
        );
      }
  ...
  update: async (req, res) => {
    try {
      ...
      if (result?.error) {
        let message = result?.error ? result?.error : ROOM.SOME_ERROR_OCCURRED;
        return errorResponse(res, message, 401, null);
      } else {
        return successResponse(
          res,
          ROOM.ROOM_UPDATED_SUCCESSFULLY,
          200,
          result.data
        );
      }
