...
const RoomService = {
    ...
    findByAttribute: async (attributes: IRoom): Promise<IRoom> => {
        try {
            const room: IRoom = await RoomModel.findOne(attributes)
                .populate('createdBy')
                .lean();
            return room;
        } catch (error) {
    ...
    deleteRoom: async (
        ...
        try {
            ...
            return {
                error:
                    error?.message || ResponseMessage.ROOM.SOME_ERROR_OCCURRED
            };
        }
    },
    /**
    ...
    list: async (
        body: QueryOptions
    ): Promise<{ rooms?: IRoom; metaData?: IMetaData; error?: string }> => {
     ...
     * @returns object of room data || {}
     */
    findMemberInRoom: async (
        attributes: IRoomMember
    ): Promise<{ error?: string; result?: IRoomMember }> => {
        try {
            return await RoomMemberModel.findOne(attributes)
                .populate('roomId')
                .populate('userId')
                .lean();
        } catch (error) {
